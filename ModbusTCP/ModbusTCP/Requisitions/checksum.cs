...
namespace ModbusTCP
{
    public static class CheckSum
    {
        ...
        private static byte[] ConvertIntToByteArray(ushort data)
        {
        ...
        public static byte[] Calculate(byte[] buffer)
        {
            try
            {
                ushort checkSumInByte = 0xFFFF;

                buffer.ToList().ForEach(byteCurrent =>
                {
                    checkSumInByte = (ushort)((checkSumInByte >> 8) ^ crcTable[(checkSumInByte ^ byteCurrent) & 0xFF]);
                });
                byte[] returnData = ConvertIntToByteArray(checkSumInByte);
                return returnData.Reverse().ToArray();
            }
            catch (Exception e)
            {
